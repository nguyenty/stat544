...
tau <-(1-bintau)*rnorm(ngene, 3, sigmatau)
...
#?jags.model
resm <- coda.samples(mm, c("tau","alpha"), 20000) # mix point mass
proc.time() -m0

plot(resm[, c("tau[1]", "tau[3]")])
tau[3]
which(tau!=0)
tau[which(tau!=0)]
alpha[1:2]
...
plot(resm[, c("alpha[4]", "alpha[49]")])
which(beta!=0)
beta[which(beta!=0)]
beta[4]
m0 <- proc.time()
inits <- list(tau = tau)
mh <- jags.model(textConnection(modelh), data,inits = inits, n.chains = 1) # horseshoe

resh <- coda.samples(mh, c("tau","alpha"), 20000) # horseshoe
proc.time() -m0

plot(resh[, c("tau[3]", "tau[1]")])
which(tau!=0)
tau[which(tau!=0)]
alpha
plot(resh[, c("alpha[1]", "alpha[2]")])
