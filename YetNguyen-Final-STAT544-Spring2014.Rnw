...
This metric rule is a modified threshold rule motivated from section 3.4 Thresholding in the paper "The Horseshoe Estimator for Sparse Signals" by Carlos M. Carvalho and Nicholas G. Polson, Biometrika (2010), 97 ,2, pp. 465–480  \url{http://ejournals.ebsco.com.proxy.lib.iastate.edu/Direct.asp?AccessToken=95M5JIX8XM9EEE91IJKU1JMX5I1D8DM4Q&Show=Object}. Note that the paper use the normal distribution setting which is different from the Poission
...
one gene, first 12 columns coresponding to line 1, the next 12 colums corresponding to 
...
coresponds to one pairs of value 
$$
...
$\mu_\tau$ of the simulation scenario. 
...
signals & 20 & 22 & 22 & 26 & 27 & 25 & 19 & 16 & 17 \\
\hline
\end{tabular}
\caption{Results of detecting signals of two models (\ref{eq1}) and (\ref{eq2})
in 9 simulation scenarios. The explanation of the table as below. Each column name is the value of $(\mu_\tau, \mu_\beta)$. Each rowname is specification of model and metric. Each element of the table indicate the number of detected signals and the number of correctly detected signals.  For example the first row "(\ref{eq1})(\ref{ev1m})" indicates model $(\ref{eq1})$ and metric $\ref{ev1m}$. The element at first row "(\ref{eq1})(\ref{ev1m})" and first column of table "19(16)" indicate the 
}
\end{table}
...
<<plottau_mh,fig.pos='H',echo=FALSE>>=
data$tau
plot(resh[, paste("tau[",1:ngene,"]",sep ="")])
@

Below is the dianogstic plots acf of the posterior samples
<<acfh,fig.pos='H',echo=FALSE>>=
par(mfrow = c(3,2))
