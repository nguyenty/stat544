...
resm <- coda.samples(mm, c("tau","beta", "alpha", "bintau", "binbeta"), 1000) 
@
...
str(data)
library(dplyr)
arrange(as.data.frame(cbind(data$line[data$gene ==mm_tau_est_eps[1]], 
...
<<echo=FALSE>>=
# dat2 <- data.frame(gene = data$gene, y= data$y, line = data$line, cov= data$cov)
# library(ggplot2)
# ggplot(data = dat2[dat2[,1] ==142, ], aes(x = cov, y = log(y+1), colour = as.factor(line))) + geom_line()
@
\end{document}