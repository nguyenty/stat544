...
\ref{fig:plottau_mh}, \ref{fig:acfh} Below is the plots obtained from an analysis based on a simulation scenario (\ref{sim})  with $(\mu_\tau, \mu_\beta) = (1,1)$ and  4 genes $j = 1,\dots, 4$, the simulated parameters $\tau_3 = 1.49$, and $\tau_j =0$ for all $j=1,2,4$. 
...
<<echo=FALSE,fig.cap='Posterior samples of $\\tau_j, j = 1,\\dots, 4$ for the real data RNA-Seq under the point mass mixture prior model (\\ref{eq1}).'>>=
...
<<tidy=FALSE,message=FALSE,echo=FALSE,results='hide'>>=
mm_tau_est_eps 
load("data.RData")
str(data)
library(dplyr)
