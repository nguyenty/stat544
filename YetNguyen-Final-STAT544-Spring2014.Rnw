...
mm_beta_est_eps <- which(apply(resm[[1]][,paste("beta[", 1:200,"]",sep ="")], 2, 
      function(x) mean(abs(x)>1)) > 0.5)
length(mm_beta_est_eps)
mm_beta_est_eps
@
...
load("data.RData")
str(data)
dat2 <- cbind(data$gene, data$y, data$line, data$cov)
dat2[dat2[,1] ==142, ]
@
