...
<<run_sim,cache=1,message=FALSE,results='hide'>>=
K <- 12
...
Looking at the results, based on the above classification criterions, it seems that the point mass mixture  model can predict very well the signals coming from lines and covariate. 


Moreover, by checking the plots of posterior densitiies of $\tau_j$ and $\beta_j$, we also have the same conclusion. Therefore, we will apply the model with point mass mixture prior to analyze the RFI data. 
