...
This rule is a modified threshold rule motivated from section 3.4 Thresholding in the paper "The Horseshoe Estimator for Sparse Signals" by Carlos M. Carvalho
 ...
 Another metric to evaluate 2 models are finding posterior probability 
...
About the evaluation rule \ref{ev2}, we choose the threshold $\varepsilon$  to be 0.5. 
...
Finally, we adopt the point mass mixture prior model to analyze the RNASeq data and obtain a list of differential expressed genes (3 genes). Note that the full RNASeq data consists of about 14000 genes, and it takes about 8 hours to run MCMC posterior samples on my computer, also the data file of posterior samples 
